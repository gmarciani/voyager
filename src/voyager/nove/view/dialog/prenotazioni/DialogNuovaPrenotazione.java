 ...
 *
 */
public class DialogNuovaPrenotazione extends JDialog{
	...
	private JTextField textfieldAcquirente;
	...
	public DialogNuovaPrenotazione(JFrame ownerFrame) {
		super(ownerFrame, "Nuova Prenotazione", true);
		...
		this.prenotazioneBean = new PrenotazioneBean();
		setDatiValidi(false);
		buildDialog();		
	}
	...
	private void buildDialog() {
		...
		this.labelAcquirente = new JLabel("Acquirente");
		...
		this.textfieldAcquirente = new JTextField("", 20);
		...
		this.panelMain.add(this.labelAcquirente, new GBCHelper(0, 1).setWeight(100, 0).setFill(GridBagConstraints.NONE).setAnchor(GridBagConstraints.WEST).setInsets(5, 15, 10, 15));
		this.panelMain.add(this.labelPostiPrenotati, new GBCHelper(0, 2).setWeight(100, 0).setFill(GridBagConstraints.NONE).setAnchor(GridBagConstraints.WEST).setInsets(5, 15, 15, 15));
		...
		this.panelMain.add(this.textfieldAcquirente, new GBCHelper(1, 1).setWeight(100, 0).setFill(GridBagConstraints.NONE).setAnchor(GridBagConstraints.WEST).setInsets(5, 15, 10, 15));
		this.panelMain.add(this.textfieldPostiPrenotati, new GBCHelper(1, 2).setWeight(100, 0).setFill(GridBagConstraints.NONE).setAnchor(GridBagConstraints.WEST).setInsets(5, 15, 15, 15));
	...
	private boolean checkSintassiDati() {
		if (this.textfieldAcquirente.getText().isEmpty() || this.textfieldPostiPrenotati.getText().isEmpty()) {
			JOptionPane.showMessageDialog(this, "Oops! Dati Mancanti!", "Nuova Prenotazione", JOptionPane.INFORMATION_MESSAGE);
			return false;
		} else {
			return true;
		}
	...
	private boolean checkSemanticaDati() {
		if (Integer.parseInt(this.textfieldPostiPrenotati.getText().toString()) <= 0) {
			JOptionPane.showMessageDialog(this, "Oops! Dati Inconsistenti!", "Nuova Prenotazione", JOptionPane.INFORMATION_MESSAGE);
			return false;
		} else {
			return true;
		}
	...
	private void setPrenotazioneBean() {
		this.prenotazioneBean.setIdOfferta(this.tabellaOfferte.getOffertaSelezionata().getId());
		this.prenotazioneBean.setAcquirente(this.textfieldAcquirente.getText().toString());
