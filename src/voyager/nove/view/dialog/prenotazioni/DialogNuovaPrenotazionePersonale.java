...
/**
 * @name DialogNuovaPrenotazionePersonale
 *
 * @project Voyager
 *
 * @package voyager.nove.view.dialog.prenotazioni
 ...
 *
 */
public class DialogNuovaPrenotazionePersonale extends JDialog{
	...
	public DialogNuovaPrenotazionePersonale(JFrame ownerFrame, UtenteBean utenteBean) {
		...
		this.prenotazioneBean = new PrenotazioneBean().setAcquirente(this.utenteBean.getUsername());
		setDatiValidi(false);
		buildDialog();		
	}
	...
	private boolean checkSintassiDati() {
		if (this.textfieldPostiPrenotati.getText().isEmpty()) {
			JOptionPane.showMessageDialog(this, "Oops! Dati Mancanti!", "Nuova Prenotazione Personale", JOptionPane.INFORMATION_MESSAGE);
			return false;
		} else {
			return true;
		}
	...
	private boolean checkSemanticaDati() {
		if (Integer.parseInt(this.textfieldPostiPrenotati.getText().toString()) <= 0) {
			JOptionPane.showMessageDialog(this, "Oops! Dati Inconsistenti!", "Nuova Prenotazione Personale", JOptionPane.INFORMATION_MESSAGE);
			return false;
		} else {
			return true;
		}
